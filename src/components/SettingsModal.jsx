...
export default function SettingsModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setStats({ totalConversations: convs.length, totalMessages: msgs, totalTokens: tokens, totalDocuments: s.totalDocuments || 0 });
      } catch {}
    })();
  }, [isOpen]);
  ...
  const estimateStorage = async () => {
    try {
      ...
      if (estimate) {
        setStorageEstimate({ usage: estimate.usage || 0, quota: estimate.quota || 0, percentUsed: estimate.quota ? Math.round((estimate.usage / estimate.quota) * 100) : 0 });
      }
    } catch {}
  };

  useEffect(() => {
    if (isOpen) { const timer = window.setTimeout(() => { void estimateStorage(); }, 0); return () => window.clearTimeout(timer); }
  }, [isOpen]);
  ...
  const handleClearAllData = async () => {
    ...
    try {
      ...
      const store = await getVectorStore(); await store.clear();
      const cacheKeys = await caches.keys();
      for (const key of cacheKeys) await caches.delete(key);
      await ai.unloadAll();
      alert(t('all_data_device')); window.location.reload();
  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      a.href = url; a.download = `local-ai-backup-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const handleImportFile = async (event) => {
    ...
    if (!file) return;
    let payload;
    try { payload = JSON.parse(await file.text()); } catch { alert("Invalid JSON."); return; }
    if (!confirm("Replace all data with this backup?")) return;
    setTransferring(true);
    try {
      const store = await getVectorStore();
      await db.delete(); await db.open(); await importAppData(payload);
      await store.clear(); await store.importData(payload.vectorStore || {});
      alert("Imported."); window.location.reload();
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md max-h-[80vh] overflow-y-auto p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={onClose} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-white transition-colors">✕</button>
          </div>

          <div className="space-y-6">
            {/* Stats */}
            <div>
              <h3 className="text-sm font-medium text-slate-300 mb-3">📊 {t('statistics')}</h3>
            ...
            {/* Accent */}
            <div>
              <h3 className="text-sm font-medium text-slate-300 mb-3">🎨 {t('accent_color')}</h3>
              <div className="flex gap-2">
                {[{ name: 'emerald', class: 'bg-emerald-500' }, { name: 'blue', class: 'bg-blue-500' }, { name: 'violet', class: 'bg-violet-500' }, { name: 'amber', class: 'bg-amber-500' }, { name: 'rose', class: 'bg-rose-500' }, { name: 'cyan', class: 'bg-cyan-500' }].map((c) => (
                  <button key={c.name} onClick={async () => { document.documentElement.setAttribute('data-accent', c.name); await setSetting('accent', c.name); }}
              ...
              {storageEstimate ? (
                <div className="bg-slate-900 rounded-xl p-4">
                  ...
                  <div className="w-full bg-slate-700 rounded-full h-2 mb-2">
                    <div className="bg-emerald-500 h-2 rounded-full transition-all" style={{ width: `${storageEstimate.percentUsed}%` }} />
                  </div>
            ...
            <div className="space-y-3">
              <button onClick={handleExportData} disabled={transferring}
              ...
              <button onClick={openImportDialog} disabled={transferring}
                className="w-full px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-lg text-sm transition-colors text-left disabled:opacity-50">
                {t('import_backup')}
              </button>
              <button onClick={() => setShowVectorManager(true)}
              ...
              <div className="border-t border-slate-700 pt-3 mt-3">
                <p className="text-xs font-medium text-slate-400 mb-2">🚀 {t('llm_server')}</p>
                <input type="text" value={serverCfg.baseUrl} onChange={e => { const n = { ...serverCfg, baseUrl: e.target.value }; setServerCfg(n); setServerConfig(n); }} placeholder="http://localhost:11434" className="w-full rounded-lg border border-slate-700 bg-slate-900/80 px-3 py-2 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-emerald-500 mb-2" />
                <input type="text" value={serverCfg.model} onChange={e => { const n = { ...serverCfg, model: e.target.value }; setServerCfg(n); setServerConfig(n); }} placeholder="llama3.2:1b" className="w-full rounded-lg border border-slate-700 bg-slate-900/80 px-3 py-2 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-emerald-500 mb-2" />
                <div className="flex items-center gap-2">
                  <button onClick={async () => { setSrvStatus({ checking: true }); const r = await checkServer(); setSrvStatus({ checking: false, result: r }); }} disabled={srvStatus.checking} className="px-3 py-1.5 text-xs bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-md disabled:opacity-50">{srvStatus.checking ? t('testing') : t('test_connection')}</button>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={serverCfg.enabled} onChange={e => { const n = { ...serverCfg, enabled: e.target.checked }; setServerCfg(n); setServerConfig(n); }} className="w-4 h-4 rounded border-slate-600" />
                    <span className="text-xs text-slate-400">{t('server_enabled')}</span>
                  </label>
                </div>
                {srvStatus.result && <div className={`text-xs p-2 mt-2 rounded ${srvStatus.result.available ? 'bg-emerald-900/30 text-emerald-400' : 'bg-red-900/30 text-red-400'}`}>{srvStatus.result.available ? t('connected') : t('not_connected')}</div>}
              </div>
            </div>

            {/* Info */}
            <div className="bg-slate-900 rounded-xl p-4 text-xs text-slate-500 space-y-1">
              <p>🔒 {t('all_data_device')}</p>
              <p>📡 {t('all_data_device')}</p>
              <p>💾 {t('all_data_device')}</p>
              <p>🧠 {t('all_data_device')}</p>
