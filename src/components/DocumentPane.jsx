...
import { BarChart3, FileText, Folder, Trash2, Paperclip, File } from 'lucide-react';
...
export default function DocumentPane() {
  ...
  const [, setTagSaveError] = useState('');
    ...
    <div className="flex flex-col md:flex-row h-full">
      <div className="w-full md:w-80 border-r border-slate-700 flex flex-col md:max-h-full max-h-[40vh]">
        <div
          ...
          onDragOver={(e) => {
            e.preventDefault();
            e.currentTarget.classList.add('bg-emerald-500/5');
          }}
          onDragLeave={(e) => {
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <label
              className={`px-3 py-1.5 text-white text-sm rounded-lg cursor-pointer transition-colors ${isProcessing ? 'bg-slate-600 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-500'}`}
            >
          ...
          <p style={{ color: 'var(--text-muted)' }} className="text-[10px] text-center mt-1">
            {t('upload')}
          </p>
          <label className="block mb-3">
            ...
            <input
              ...
              className="w-full rounded-lg border px-3 py-2 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/30"
            />
          </label>
          {isProcessing && (
            <div
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          ...
          {filteredDocuments.map((doc) => (
            <div
              ...
              className={`rounded-lg p-3 cursor-pointer transition-colors hover:bg-slate-750 ${selectedDoc === doc.id ? 'ring-2 ring-emerald-500' : ''}`}
            >
              <div className="flex items-center gap-3">
                <span style={{ color: 'var(--text-muted)' }} className="text-xl">
        ...
        {selectedDoc && docContent ? (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 style={{ color: 'var(--text-primary)' }} className="text-lg font-medium">
            ...
            >
              ...
              <div className="flex gap-2 mt-2">
                <textarea
                  ...
                  className="w-full rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-500"
                />
                <button
                  onClick={handleSaveTags}
                  className="rounded-lg bg-emerald-600 hover:bg-emerald-500 px-3 py-2 text-xs font-medium text-white"
                >
                  {t('save')}
                </button>
              </div>
            ...
            {selectedDocument?.fileType === 'application/pdf' ? (
              ...
              >
                <pre
                  ...
                  className="text-sm whitespace-pre-wrap font-sans leading-relaxed p-6"
                >
                  {docContent.slice(0, DOC_PREVIEW_MAX_CHARS)}
                </pre>
                {docContent.length > DOC_PREVIEW_MAX_CHARS && (
                  <p style={{ color: 'var(--text-muted)' }} className="text-xs px-6 pb-6">
          ...
          <div className="flex items-center justify-center h-full">
            ...
            <p style={{ color: 'var(--text-muted)' }} className="text-sm ml-3">
              {t('no_documents')}
            </p>
          </div>
        )}
